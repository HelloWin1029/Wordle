 ...
 *
 *	@author	Scott DeRuiter and David Greenstein and Win Chang
 *	@version	1.0
...
{ 
	...
	{
		String result = "SMART";
		if (testWord.length() == 5 && inAllowedWordFile(testWord) == true) result = testWord;
		else{
			int numOfLine = numOfLineInFile();
			int chosenLine = (int)(Math.random() * numOfLine) + 1;
			result = findWordInFile(chosenLine);
		}
		
		if (show == true) System.out.println(result.toUpperCase());
		return result;
	}
	
	/** 
	 *	Count the number of line of input strings in the text file words5allowed.txt
	 * 	@return 	the number of line of input strings in the text file words5allowed.txt
	 */
	public int numOfLineInFile(){
		java.util.Scanner input = null;
		input = FileUtils.openToRead("words5allowed.txt");
	...
	/** 
	 * 	Find the string in the text file words5allowed.txt given the line it's on.
	 * 	@param		the line number in which the return string should be on in the text file.
	 * 	@return 	the string on the given line in the text file.
	 */
	public String findWordInFile(int chosenLine){
		java.util.Scanner input = null;
		input = FileUtils.openToRead("words5allowed.txt");
		
		String result = "";
		for (int i = 1; i <= chosenLine; i++) result = input.nextLine();
		input.close();
	...
	public boolean inAllowedWordFile(String possibleWord){
		...
		boolean found = false;
		while(input.hasNext()){
			String wordAllowed = input.nextLine();
			if (wordAllowed.equalsIgnoreCase(possibleWord)) found = true;
		}
		input.close();
